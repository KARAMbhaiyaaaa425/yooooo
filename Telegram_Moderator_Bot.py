...
async def unban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.unban_chat_member(chat_id, target.id)
        await update.message.reply_text(f"✅ Unbanned {target.mention_html()}", parse_mode="HTML")
    except Exception as e:
        await update.message.reply_text(f"Failed: {e}")
...
async def mute_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"🔇 Muted {target.mention_html()} for {duration} seconds.", parse_mode="HTML")
    except Exception as e:
        await update.message.reply_text(f"Failed: {e}")
...
async def warn_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if count >= max_w:
        ...
        msg = f"⚠️ Warning {count}/{max_w} for {target.mention_html()}.\nMaximum reached! Action: {action}."
        try:
            if action == 'kick':
                await context.bot.ban_chat_member(chat_id, target.id)
                await context.bot.unban_chat_member(chat_id, target.id)
        ...
        await update.message.reply_text(f"⚠️ Warning {count}/{max_w} for {target.mention_html()}.", parse_mode="HTML")
            ...
            if count >= 3:
                ...
                try:
                    ...
                    await context.bot.send_message(
                        chat_id, 
                        f"🚫 {msg.from_user.mention_html()} has been muted for {mins} minutes for repeatedly posting links!\n\nTo get unmuted early, DM the Owner.", 
                        parse_mode="HTML"
                    )
                except Exception as e:
                ...
                try:
                    await context.bot.send_message(
                        chat_id, 
                        f"⚠️ {msg.from_user.mention_html()}, links are not allowed here! (Warning {count}/2)", 
                        parse_mode="HTML"
                    )
                except:
                    pass
    ...
    if settings['anti_flood']:
        ...
        if len(cache['times']) >= settings['anti_flood_limit']:
            try: 
                ...
                await context.bot.send_message(chat_id, f"🚫 Muted {msg.from_user.mention_html()} for 5m due to flooding.", parse_mode="HTML")
            except: 
                pass
            cache['times'] = []
...
async def set_button_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if name.lower() == "remove":
        ...
        await update.message.reply_text("✅ Button removed!")
    else:
...
def get_settings_keyboard(target_chat_id):
    ...
    kb = [
        [
            InlineKeyboardButton(f"Welcome System: {'✅' if settings['welcome_enabled'] else '❌'}", callback_data=f"tg_we_{target_chat_id}"),
            InlineKeyboardButton(f"Anti-Link: {'✅' if settings['anti_link'] else '❌'}", callback_data=f"tg_al_{target_chat_id}")
        ],
        [
            InlineKeyboardButton(f"Auto-Accept Join Requests: {'✅' if settings.get('auto_accept_joins') else '❌'}", callback_data=f"tg_aa_{target_chat_id}")
        ],
        [
            InlineKeyboardButton(f"Anti-Flood: {'✅' if settings['anti_flood'] else '❌'}", callback_data=f"tg_af_{target_chat_id}"),
            InlineKeyboardButton(f"Bad Words: {'✅' if settings['word_blacklist'] else '❌'}", callback_data=f"tg_wb_{target_chat_id}")
        ],
        [
            InlineKeyboardButton(f"Block Photos: {'❌' if settings['media_photo'] else '✅'}", callback_data=f"tg_mp_{target_chat_id}"),
            InlineKeyboardButton(f"Block Videos: {'❌' if settings['media_video'] else '✅'}", callback_data=f"tg_mv_{target_chat_id}")
        ],
        [
            InlineKeyboardButton(f"Block GIFs: {'❌' if settings['media_gif'] else '✅'}", callback_data=f"tg_mg_{target_chat_id}"),
        ...
        [
            InlineKeyboardButton("🔊 Unmute a User", callback_data=f"tg_unm_{target_chat_id}"),
            InlineKeyboardButton("👑 Promote to Admin", callback_data=f"tg_mkad_{target_chat_id}")
        ],
        [
...
async def settings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"⚙️ <b>Settings for {update.effective_chat.title}</b>", reply_markup=get_settings_keyboard(chat_id), parse_mode="HTML")
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "tg_close":
        if str(user_id) == "8373276191":
            ...
            kb.append([InlineKeyboardButton("📝 Set Bot Start Message", callback_data="tg_gstart")])
            await query.edit_message_text("🏢 <b>Your Groups</b>\nSelect a group to manage:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
            return
        else:
    ...
    if data == "tg_gstart":
        ...
        await query.message.reply_text("🤖 Please send or forward the message you want to set as the Bot's Start Message.\nType /cancel to abort.")
        return
    ...
    if len(parts) >= 3:
        ...
        if key_code == 'wl':
            ...
            en_status = '✅ ON' if settings.get('welcome_enabled', 1) else '❌ OFF'
            media_status = "🖼 Set" if settings.get('welcome_media_id') else "Not Set"
            voice_status = "🎙 Set" if settings.get('welcome_voice_id') else "Not Set"
            
            kb = [
                [InlineKeyboardButton(f"Welcome System: {en_status}", callback_data=f"tg_wlen_{target_id}")],
                [InlineKeyboardButton("➕ Set Text", callback_data=f"tg_sw_{target_id}")],
                [InlineKeyboardButton(f"➕ Set Photo/Video ({media_status})", callback_data=f"tg_wlmd_{target_id}")],
                [InlineKeyboardButton(f"➕ Set Voice ({voice_status})", callback_data=f"tg_sv_{target_id}")],
                [InlineKeyboardButton("🗑 Remove Photo/Video", callback_data=f"tg_wlrmd_{target_id}")],
                [InlineKeyboardButton("🗑 Remove Voice", callback_data=f"tg_rv_{target_id}")],
                [InlineKeyboardButton("🔙 Back to Settings", callback_data=f"tg_open_{target_id}")]
            ]
            await query.edit_message_text("👋 <b>Welcome Message Management</b>\nConfigure how new members are greeted:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
            return
        ...
        if key_code == 'wlmd':
            ...
            await query.message.reply_text("🖼 Please send a **Photo** or **Video** you want to attach to the Welcome Message now.\nType /cancel to abort.", parse_mode="Markdown")
            return
        ...
        if key_code == 'sw':
            ...
            await query.message.reply_text("✏️ Please send the new Welcome Message for this group now.\n(Supports {name} and {group})\nType /cancel to abort.")
            return
            
        if key_code == 'sv':
            context.user_data['awaiting_welcome_voice'] = target_id
            await query.message.reply_text("🎙 Please send or forward the Voice Note for the Welcome Message now.\nType /cancel to abort.")
            return
        ...
        if key_code == 'bc':
            ...
            await query.message.reply_text("📢 Please forward or send the message you want to broadcast to this group now.\nType /cancel to abort.")
            return
            
        if key_code == 'unm':
            context.user_data['awaiting_unmute'] = target_id
            await query.message.reply_text("🔊 Please send the **User ID** OR **Username** (e.g. @sagar) of the user you want to unmute.\nType /cancel to abort.", parse_mode="Markdown")
            return
            
        if key_code == 'mkad':
            context.user_data['awaiting_admin'] = target_id
            await query.message.reply_text("👑 Please send the **User ID** OR **Username** (e.g. @sagar) of the user you want to promote to Admin.\nType /cancel to abort.", parse_mode="Markdown")
            return
            
        if key_code == 'ar':
            kb = [
                [InlineKeyboardButton("➕ Add Text Reply", callback_data=f"tg_artxt_{target_id}")],
                [InlineKeyboardButton("➕ Add Voice Reply", callback_data=f"tg_arvc_{target_id}")],
                [InlineKeyboardButton("➕ Add Sticker/GIF Reply", callback_data=f"tg_arst_{target_id}")],
                [InlineKeyboardButton("🗑 View & Delete Auto-Replies", callback_data=f"tg_arls_{target_id}")],
                [InlineKeyboardButton("🔙 Back to Settings", callback_data=f"tg_open_{target_id}")]
            ]
            await query.edit_message_text(f"🤖 <b>Auto-Replies Management</b>", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
            return
        ...
        if key_code in ['artxt', 'arvc', 'arst']:
            ...
            await query.message.reply_text("1️⃣ Send the **Trigger Word** (e.g. 'help'). If anyone says this in the group, I will reply.\nType /cancel to abort.", parse_mode="Markdown")
            return
        ...
        if key_code == 'arls':
            ...
            kb = []
            for r in replies:
                kb.append([InlineKeyboardButton(f"❌ Delete '{r['trigger_text']}'", callback_data=f"tg_ardl_{target_id}_{r['trigger_text']}")])
            kb.append([InlineKeyboardButton("🔙 Back", callback_data=f"tg_ar_{target_id}")])
            await query.edit_message_text("🗑 Select an Auto-Reply to delete:", reply_markup=InlineKeyboardMarkup(kb))
            return
        ...
        if key_code == 'ardl':
            ...
            kb.append([InlineKeyboardButton("🔙 Back", callback_data=f"tg_ar_{target_id}")])
            if not replies:
                kb = [[InlineKeyboardButton("🔙 Back", callback_data=f"tg_ar_{target_id}")]]
            await query.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(kb))
            return
    ...
    if update.effective_chat.type == "private":
        ...
        if target_id_bc:
            ...
            try:
                ...
                await update.message.reply_text("✅ Message Broadcasted and Pinned to the group successfully!")
            except Exception as e:
        ...
        if ar_state:
            ...
            if ar_state == 'trigger' and update.message.text:
                ...
                if ar_type == 'artxt':
                    ...
                    await update.message.reply_text("2️⃣ Send the **Text Message** you want the bot to reply with.")
                elif ar_type == 'arvc':
                    context.user_data['ar_state'] = 'reply_only_voice'
                    await update.message.reply_text("2️⃣ Send the **Voice Note** you want the bot to reply with.")
                elif ar_type == 'arst':
                    context.user_data['ar_state'] = 'reply_only_sticker'
                    await update.message.reply_text("2️⃣ Send the **Sticker** or **GIF** you want the bot to reply with.")
                return
            ...
            if ar_state == 'reply_only_text' and update.message.text:
                ...
                context.user_data['ar_state'] = None
                await update.message.reply_text("✅ Text Auto-Reply saved successfully!")
                return
            ...
            if ar_state == 'reply_only_voice' and update.message.voice:
                ...
                context.user_data['ar_state'] = None
                await update.message.reply_text("✅ Voice Auto-Reply saved successfully!")
                return
            ...
            if ar_state == 'reply_only_sticker' and (update.message.sticker or update.message.animation):
                ...
                context.user_data['ar_state'] = None
                await update.message.reply_text("✅ Sticker/GIF Auto-Reply saved successfully!")
                return
                
            if ar_state in ['reply_only_text', 'reply_only_voice', 'reply_only_sticker']:
                await update.message.reply_text("❌ You sent the wrong type of message for this option. Please send the correct media type or type /cancel.")
                return
            return
        ...
        if target_id_md:
            ...
            if update.message.photo:
                ...
                context.user_data['awaiting_welcome_media'] = None
                await update.message.reply_text("✅ Welcome Photo saved successfully!")
                ...
                context.user_data['awaiting_welcome_media'] = None
                await update.message.reply_text("✅ Welcome Video saved successfully!")
            else:
                await update.message.reply_text("❌ Please send a Photo or Video, or type /cancel.")
            return
            ...
            try:
                ...
                if raw.startswith('@') or not raw.isdigit():
                    ...
                    if not user_id_to_unmute:
                        await update.message.reply_text(f"❌ User '{raw}' not found in database. Please use their User ID instead.")
                        return
                else:
                ...
                context.user_data['awaiting_unmute'] = None
                await update.message.reply_text(f"✅ User {raw} has been successfully unmuted and their warning count reset!")
            except Exception as e:
        ...
        if target_id_ad and update.message.text:
            ...
            try:
                ...
                context.user_data['awaiting_admin'] = None
                await update.message.reply_text(f"👑 User {raw} has been promoted to Admin in the group!")
            except Exception as e:
        ...
        if context.user_data.get('awaiting_global_start'):
            ...
            context.user_data['awaiting_global_start'] = False
            await update.message.reply_text("✅ Bot Start Message saved successfully! Anyone starting the bot will receive this exact message.")
            return
